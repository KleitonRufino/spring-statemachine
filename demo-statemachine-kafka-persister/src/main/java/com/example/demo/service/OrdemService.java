...
public class OrdemService {
	...
	public void executar() throws Exception {
		...
		for (Ordem ordem : list) {
			...
			log.info("START MAQUINA DE ESTADO NO ESTADO " + machine.getState().getId());
			machine.start();
	...
	public void executarComErro() {
		...
		for (Ordem ordem : list) {
			try {
				StateMachine<States, Events> machine = machineService.getStateMachine(String.valueOf(ordem.getId()));
				log.info("START MAQUINA DE ESTADO NO ESTADO " + machine.getState().getId());
				machine.start();
