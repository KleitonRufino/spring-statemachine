...
public class OrdemService {
	...
	@Autowired
	private StMachineService stMachineService;

	public void executar() throws Exception {

		List<Ordem> list = findAllByStatus(Status.WAITING);
		for (Ordem ordem : list) {
			StateMachine<States, Events> machine = stMachineService.getStateMachine(String.valueOf(ordem.getId()));
		
			//machine.sendEvent(Mono.just(MessageBuilder.withPayload(Events.UPDATING_STATUS).build()));
			machine.sendEvent(Events.UPDATING_STATUS);
			
			ordem.setStatus(Status.PROCESSED);
			save(ordem);

//			machine.sendEvent(Mono.just(MessageBuilder.withPayload(Events.UPDATED_STATUS).build()));
